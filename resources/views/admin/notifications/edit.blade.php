                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.notifications.update", [$notification->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('sub_title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="sub_title" id="sub_title" value="{{ old('sub_title', $notification->sub_title) }}">
                            @if($errors->has('sub_title'))
                                <span class="help-block" role="alert">{{ $errors->first('sub_title') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.notification.fields.sub_title_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('link') ? 'has-error' : '' }}">
                            <label class="required" for="link">{{ trans('cruds.notification.fields.link') }}</label>
                            <input class="form-control" type="text" name="link" id="link" value="{{ old('link', $notification->link) }}" required>
                            @if($errors->has('link'))
                        ...
                        <div class="form-group {{ $errors->has('icon') ? 'has-error' : '' }}">
                            <label class="required" for="icon">{{ trans('cruds.notification.fields.icon') }}</label>
                        ...
                        <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                            <label for="image">{{ trans('cruds.notification.fields.image') }}</label>
                            <div class="needsclick dropzone" id="image-dropzone">
                            </div>
                            @if($errors->has('image'))
                                <span class="help-block" role="alert">{{ $errors->first('image') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.notification.fields.image_helper') }}</span>
                        </div>
...
@if(isset($notification) && $notification->image)
      var file = {!! json_encode($notification->image) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview)
      file.previewElement.classList.add('dz-complete')
