                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.applications.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('one_signal_account') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="one_signal_account_id" id="one_signal_account_id" required>
                                @foreach($one_signal_accounts as $id => $entry)
                                    <option value="{{ $id }}" {{ old('one_signal_account_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('onesignal_app') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="onesignal_app" id="onesignal_app" value="{{ old('onesignal_app', '') }}" required>
                        ...
                        <div class="form-group {{ $errors->has('rest_api_key') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="rest_api_key" id="rest_api_key" value="{{ old('rest_api_key', '') }}" required>
